...
use crate::{
	bits_to_rate, keccakp::KeccakP, EncodedLen, Hasher, IntoXof, KeccakState,
	Xof,
};
...
impl<T: AsRef<[u8]>> Hasher for KangarooTwelve<T> {
	fn update(&mut self, input: &[u8]) {
		...
		if self.chunks == 0 {
			let todo = core::cmp::min(
				Self::MAX_CHUNK_SIZE - self.written,
				to_absorb.len(),
			);
...
impl<T: AsRef<[u8]>> IntoXof for KangarooTwelve<T> {
	...
	fn into_xof(mut self) -> KangarooTwelveXof {
		let custom_string = self.custom_string.take().expect(
			"KangarooTwelve cannot be initialized without custom_string; qed",
		);
