...
use crate::{
	bits_to_rate, keccakf::KeccakF, EncodedLen, Hasher, KeccakState, Xof,
};
...
impl CShake {
	...
	pub(crate) fn new(
		...
		state.update(EncodedLen::left_encode(name.len() * 8).value());
		state.update(name);
		state.update(EncodedLen::left_encode(custom_string.len() * 8).value());
